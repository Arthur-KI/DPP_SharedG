...
class FPLA_V1_Dataset(IterableDataset): # Umbenannt von StackMachineV1Dataset für Klarheit
    def __init__(self, num_sequences_per_epoch, seq_len, noise_level=0.0,
        ...
        self.LOAD_RA_X = 2
        self.MOVE_R0_R1 = 3
        self.ALU_XOR_R0_R1 = 4
        self.ALU_AND_R0_R1 = 5
        self.ALU_NOT_R0 = 6
        self.STORE_R0_AT_RA = 7
        self.LOAD_R0_FROM_RA = 8
        self.SET_F0_IF_R0_ZERO = 9
        self.JUMP_IF_F0_SKIP_N = 10 # N ist fest (z.B. 3)
        self.CALL_SUB_FIXED = 11
        ...
        assert len(self.instr_names) == self.num_instructions, "Mismatch in instruction names and count"

    # KORREKTUR: _to_one_hot Methode hier definieren
    ...
    def __iter__(self):
        ...
        for _ in range(self.num_sequences_per_epoch):
            ...
            while idx_in_sequence < self.seq_len:
                ...
                ra_one_hot = self._to_one_hot(ra, self.data_mem_size)
...
if __name__ == "__main__":
    ...
    num_instructions_task = 16 # Gemäß Definition im Dataset
    data_mem_size_task = 4
    ret_stack_depth_task = 4
    
    # Input: x1(1) + instr_oh(16) + R0(1) + R1(1) + RA_val_oh(4) + F0(1) + DataMem(4) + SP_oh(4) + y_tm1(1) = 33
    input_size = 1 + num_instructions_task + 1 + 1 + data_mem_size_task + 1 + data_mem_size_task + ret_stack_depth_task + 1

    output_size = 1
    learning_rate = 0.001 
    batch_size = 2048 
    epochs = 5 # Aus Ihrem Log, Sie hatten es auf 5 reduziert. Für einen echten Test ggf. erhöhen.
    noise_level_data = 0.001 
    target_accuracy_threshold = 0.65

    dpp_units = 64 # Aus Ihrem Log
    shared_g_dim_config = 13 # Aus Ihrem Log
    ...
    print(f"\nModellparameter: {param_count}") # Erwartet 4909 basierend auf Ihrem Log
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', patience=30, factor=0.25, min_lr=1e-8, verbose=True) # Patience erhöht

    print(f"\n--- Training DPP SharedG (H={dpp_units}, SG_dim={shared_g_dim_config}) für FPLA V1 ---")
    # Sie hatten epochs auf 5 reduziert, hier können Sie es wieder auf den geplanten Wert setzen
    # epochs_this_run = 5 # Gemäß Ihrem Log
    epochs_this_run = epochs # Für einen längeren Lauf
    ...
    # Simulation der Zustände für eine kohärente Inspektion
    insp_r0, insp_r1, insp_ra_val, insp_f0 = 0.0, 0.0, 0.0, 0.0 # RA als Wert
    ...
    with torch.no_grad():
        for sample_count_inspect in range(max_inspect_alpha_fpla_v1):
            ...
            current_ra_oh_clean = inspector_dataset_fpla_v1._to_one_hot(insp_ra_val, data_mem_size_task) # RA als One-Hot für Input
            ...
            if len(model_input_features_clean) != input_size:
                print(f"WARNUNG: Feature-Anzahl in Inspektion ({len(model_input_features_clean)}) vs. input_size ({input_size}).")
                continue
            ...
            # Berechne korrekten nächsten Zustand und y_t
            next_insp_r0, next_insp_r1, next_insp_ra, next_insp_f0 = insp_r0, insp_r1, insp_ra_val, insp_f0
            ...
            print(f"S{sample_count_inspect+1}: x1:{int(current_insp_x1_clean)},I:{instr_display_name}({current_instr_idx_clean_insp})|R0i:{int(insp_r0)},R1i:{int(insp_r1)},RAi:{int(insp_ra_val)},F0i:{int(insp_f0)},DM:[{dm_str}],SPi:{insp_ret_sp},y-1i:{int(insp_previous_y)}|Py:{int(model_output_y)},Ty:{int(correct_target_y_insp)},MA:{mean_alpha:.3f}")
