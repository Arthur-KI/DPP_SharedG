...
SEED = 58 # Beibehalten von Test20.py für Konsistenz mit dem "CPU-Kern"-Test
...
# --- Modell-Definitionen (DPPModelBase leicht angepasst für Alpha-Retrieval in predict) ---
class DPPModelBase(nn.Module):
    def __init__(self, dpp_layer, hidden_size_dpp, output_size):
        ...
        self.last_alphas = None # Wird jetzt auch in predict_next_state_and_output gesetzt
    ...
    def forward(self, x, return_alpha_flag=False):
        if return_alpha_flag and hasattr(self.dpp_layer1, 'forward') and \
           'return_alpha' in self.dpp_layer1.forward.__code__.co_varnames:
            ...
            self.last_alphas = alphas # Speichere Alphas, wenn return_alpha_flag True ist
        else:
            # Auch wenn return_alpha_flag False ist, aber wir für die Vorhersage Alphas brauchen,
            # könnten wir sie hier intern abrufen, falls der Layer es unterstützt.
            # Für Konsistenz: last_alphas wird nur aktualisiert, wenn explizit angefordert.
            if hasattr(self.dpp_layer1, 'forward') and 'return_alpha' in self.dpp_layer1.forward.__code__.co_varnames:
                 out, alphas_for_pred = self.dpp_layer1(x, return_alpha=True) # Intern immer mit Alpha holen
                 self.last_alphas = alphas_for_pred # Aktualisiere immer, wenn möglich
            else:
    ...
    def predict_next_state_and_output(self, model_input_features_clean_tensor):
        self.eval()
        # Rufe forward so auf, dass Alphas intern gespeichert werden
        output_logit = self(model_input_features_clean_tensor, return_alpha_flag=True) # return_alpha_flag=True setzen
        predicted_y_t = (torch.sigmoid(output_logit) > 0.5).float().item()
        # last_alphas sollte jetzt durch den forward-Aufruf gesetzt sein
    ...
    def __iter__(self):
        for _ in range(self.num_sequences_per_epoch):
            ...
            while idx_in_sequence < self.seq_len:
                ...
                elif current_instr_idx_clean == self.ALU_XOR_R1R2_R0: res = float(int(r[1]) ^ int(r[2])); next_r[0] = res; next_zf = 1.0 if res == 0 else 0.0; next_eqf = 1.0 if int(r[1])==int(r[2]) else 0.0
                elif current_instr_idx_clean == self.ALU_AND_R1R2_R0: res = float(int(r[1]) & int(r[2])); next_r[0] = res; next_zf = 1.0 if res == 0 else 0.0; next_eqf = 1.0 if int(r[1])==int(r[2]) else 0.0
                elif current_instr_idx_clean == self.ALU_OR_R1R2_R0: res = float(int(r[1]) | int(r[2])); next_r[0] = res; next_zf = 1.0 if res == 0 else 0.0; next_eqf = 1.0 if int(r[1])==int(r[2]) else 0.0
                elif current_instr_idx_clean == self.ALU_NOT_R1_R0: res = float(1 - int(r[1])); next_r[0] = res; next_zf = 1.0 if res == 0 else 0.0
...
# --- Trainings- und Evaluierungsfunktionen (angepasst, um bestes Modell zu speichern) ---
def train_model_amp(model, train_loader, criterion, optimizer, scaler, epochs=100, model_name="Model", target_accuracy=0.98, steps_per_epoch=None, scheduler_obj=None):
    ...
    early_stop_threshold = 20 # Wie in Test20.py
    ...
    for epoch in range(epochs):
        ...
        if accuracy >= 0.999: # Schwelle für Early Stopping
            consecutive_target_epochs += 1
            if consecutive_target_epochs >= early_stop_threshold:
                print(f"--- {model_name} erreichte stabile hohe Genauigkeit für {early_stop_threshold} Epochen. Training wird frühzeitig in Epoche {epoch+1} beendet. ---")
                break
        else:
            consecutive_target_epochs = 0
...
class CPUSimulator:
    def __init__(self, num_instructions=18, data_mem_size=8, ret_stack_depth=4, pc_bits=6):
        ...
        ds = CPU_Core_V0_1_Dataset(1,1, num_instructions=num_instructions, data_mem_size=data_mem_size, ret_stack_depth=ret_stack_depth, pc_bits=pc_bits)
    ...
    def get_model_input_features(self, instr_idx_clean, control_data_bits_clean):
        ...
        input_features_clean = control_data_bits_clean + \
                               list(instruction_one_hot) + self.r + list(ar_one_hot) + \
                               [self.zf, self.eqf] + self.data_memory + list(sp_one_hot) + \
                               [self.previous_y]
        return torch.tensor([input_features_clean], dtype=torch.float32).to(DEVICE)


def execute_program_on_model_and_sim(model, program_dict, simulator, max_steps=100):
    model.eval()
    simulator.reset_state()
    
    print("\n--- Programmausführung Start ---")
    header = f"{'SimPC':>5} | {'Instr':<7} | {'Ctrl':<6} | {'R0-3':<10} | {'AR':>2} | {'ZF':>2} | {'EQF':>3} | {'Mem':<17} | {'SP':>2} | {'StkTop':>6} | {'y_prev':>6} | {'Exp_Y':>5} | {'Mod_Y':>5} | {'Alpha':>5}"
    print(header)
    print("-" * len(header))
    ...
    for step_count in range(max_steps):
        ...
        if current_pc_sim not in program_dict:
            print(f"PC {current_pc_sim} nicht im Programm definiert. Programmausführung beendet.")
            break
            
        instr_idx, control_bits = program_dict[current_pc_sim]
        ...
        state_before_exec = simulator.history[-1] # Der Zustand *vor* der Ausführung

        if int(predicted_y) == int(expected_y):
            correct_y_predictions += 1
        executed_steps += 1
        
        print(f"{state_before_exec['pc']:>5} | {state_before_exec['instr']:<7} | {''.join(map(str,state_before_exec['ctrl_bits'])) :<6} | {str(state_before_exec['r']):<10} | {state_before_exec['ar']:>2} | {state_before_exec['zf']:>2} | {state_before_exec['eqf']:>3} | {str(state_before_exec['mem']):<17} | {state_before_exec['sp']:>2} | {state_before_exec['stack_top']:>6} | {state_before_exec['y_prev']:>6} | {int(expected_y):>5} | {int(predicted_y):>5} | {mean_alpha:.3f}")

        if instr_idx == simulator.NO_OP and current_pc_sim == simulator.program_counter : # Einfache Endebedingung, wenn PC auf NOOP zeigt und sich nicht ändert
             if program_dict.get(current_pc_sim +1) is None: # Und keine weitere Instruktion definiert ist
                print("Programmende durch NOOP erreicht.")
                break
    
    if executed_steps == 0:
        accuracy = 0.0
        print("Keine Schritte im Programm ausgeführt.")
    else:
        accuracy = (correct_y_predictions / executed_steps) * 100
    print("-" * len(header))
    print(f"Programmausführung beendet nach {executed_steps} Schritten. Genauigkeit der y_t Vorhersagen: {accuracy:.2f}% ({correct_y_predictions}/{executed_steps})")
    return accuracy
...
if __name__ == "__main__":
    ...
    batch_size = 4096
    epochs = 10 # Erhöht für besseres Training vor dem "harten Test"
    noise_level_data = 0.001
    target_accuracy_threshold = 0.95 # Etwas höheres Ziel für das Training

    dpp_units = 128
    shared_g_dim_config = 25

    print(f"Task: CPU-Kern V0.1 - Programmausführungstest (Härterer Test)")
    ...
    num_train_sequences_per_epoch = 100000 # Mehr Trainingsdaten
    num_test_sequences_per_epoch = 20000
    seq_len_task = 60 # Längere Sequenzen für das Training
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', patience=7, factor=0.2, min_lr=1e-9, verbose=True)
    ...
    history, total_time, epoch_target, time_target = train_model_amp(
        ...
        epochs=epochs, model_name="DPP_CPU_Core_V0_1_HardTest", target_accuracy=target_accuracy_threshold,
        steps_per_epoch=train_steps_per_epoch, scheduler_obj=scheduler
    )

    print("\n--- Finale Evaluation des trainierten Modells (auf Zufallsdaten) ---")
    final_acc, final_loss = evaluate_model_amp(model_to_train, test_loader, criterion, model_name="DPP_CPU_Core_V0_1_HardTest", steps_per_epoch=test_steps_per_epoch)
    print(f"DPP_CPU_Core_V0_1_HardTest - Parameter: {param_count}")
    print(f"  Final Training Accuracy (Ende letzter Epoche): {history['accuracy'][-1]:.4f}")
    print(f"  Final Test Accuracy: {final_acc:.4f}")
    print(f"  Final Test Loss: {final_loss:.4f}")

    # --- Härteres Testprogramm definieren und ausführen ---
    ...
    LDR0X = cpu_sim.LOAD_R0_X; LDR1X = cpu_sim.LOAD_R1_X; LDR2X = cpu_sim.LOAD_R2_X
    LDARX = cpu_sim.LOAD_AR_X; STOMEM = cpu_sim.STORE_R0_MEM_AR
    LDMEM = cpu_sim.LOAD_R0_MEM_AR; OUTR0 = cpu_sim.OUT_R0; XOR = cpu_sim.ALU_XOR_R1R2_R0
    AND = cpu_sim.ALU_AND_R1R2_R0; OR = cpu_sim.ALU_OR_R1R2_R0; NOT = cpu_sim.ALU_NOT_R1_R0
    CALL = cpu_sim.CALL_ADDR; RET = cpu_sim.RETURN; JMPZF = cpu_sim.JUMP_IF_ZF_ADDR
    NOOP = cpu_sim.NO_OP

    # SUBROUTINE_ADDRESS muss als PC-Wert im Program-Dictionary existieren
    SUBROUTINE_ADDRESS = 20 # PC 20
    # Adresse für CALL kodieren (z.B. SUBROUTINE_ADDRESS = 20 -> binär 010100)
    # control_bits für CALL: [c0, c1, c2, c3, c4, c5] -> [0,0,1,0,1,0] für Adresse 20
    ctrl_call_sub20 = [0,0,1,0,1,0]

    # Adresse für JUMP_IF_ZF_ADDR (z.B. PC 7 -> binär 000111)
    # control_bits für JMPZF: [c0, c1, c2, c3, c4, c5] -> [1,1,1,0,0,0] für Adresse 7
    ctrl_jmp_pc7 = [1,1,1,0,0,0]


    # Programm als Dictionary: {PC: (INSTRUKTION, [val/addr0, addr1, addr2, c3, c4, c5])}
    test_program_advanced_dict = {
        # Hauptprogramm
        0: (LDR0X,   [1,0,0,0,0,0]), # R0 = 1
        1: (LDR1X,   [0,0,0,0,0,0]), # R1 = 0 (R2 bleibt 0)
        2: (LDR2X,   [1,0,0,0,0,0]), # R2 = 1 (für XOR R1,R2)
        3: (LDARX,   [0,0,0,0,0,0]), # AR = 0
        4: (STOMEM,  [0,0,0,0,0,0]), # Mem[0] = R0 (1) (speichere R0 original)
        5: (CALL,    ctrl_call_sub20),# CALL SUBROUTINE_ADDRESS (PC=20). Stack: PC+1=6
        # Nach Rückkehr von Subroutine ist PC = 6
        6: (LDARX,   [1,0,0,0,0,0]), # AR = 1
        7: (STOMEM,  [0,0,0,0,0,0]), # Mem[1] = R0 (R0 ist der Wert aus der Subroutine: NOT(R1_vor_call XOR R2_vor_call))
                                    # R1_vor_call=0, R2_vor_call=1.  0 XOR 1 = 1. NOT 1 = 0. Also R0=0. Mem[1]=0.
        8: (LDR0X,   [0,0,0,0,0,0]), # Lade R0 mit 0, um ZF zu testen
        9: (XOR,     [0,0,0,0,0,0]), # R0 = R1(0) XOR R2(1) = 1. R0 ist 1. ZF=0.
        10: (JMPZF,  ctrl_jmp_pc7),  # ZF ist 0, Sprung zu PC=7 wird NICHT ausgeführt. PC wird 11.
        11: (LDARX,   [0,0,0,0,0,0]), # AR = 0
        12: (LDMEM,   [0,0,0,0,0,0]), # R0 = Mem[0] (der ursprüngliche R0-Wert, 1)
        13: (OUTR0,   [0,0,0,0,0,0]), # Output R0 (sollte 1 sein)
        14: (LDARX,   [1,0,0,0,0,0]), # AR = 1
        15: (LDMEM,   [0,0,0,0,0,0]), # R0 = Mem[1] (der Wert aus der Subroutine, 0)
        16: (OUTR0,   [0,0,0,0,0,0]), # Output R0 (sollte 0 sein)
        17: (NOOP,    [0,0,0,0,0,0]), # Programm-Ende Markierung (optional)

        # Subroutine bei SUBROUTINE_ADDRESS (PC=20)
        SUBROUTINE_ADDRESS + 0: (NOT,     [0,0,0,0,0,0]), # R0 = NOT R1 (R1 ist 0 -> R0=1) (Hier ist R1 der erste Operand für NOT)
        SUBROUTINE_ADDRESS + 1: (LDR2X,   [1,0,0,0,0,0]), # R2 = 1 (Als zweiter Operand für XOR R1,R2)
        SUBROUTINE_ADDRESS + 2: (XOR,     [0,0,0,0,0,0]), # R0 = R1(0) XOR R2(1) = 1.
        SUBROUTINE_ADDRESS + 3: (LDARX,   [0,0,0,0,0,0]), # AR = 0
        SUBROUTINE_ADDRESS + 4: (STOMEM,  [0,0,0,0,0,0]), # Mem[AR] (Mem[0]) = R0 (1)
        SUBROUTINE_ADDRESS + 5: (RET,     [0,0,0,0,0,0]), # Return to PC=6 (vom Stack)
    }

    execute_program_on_model_and_sim(model_to_train, test_program_advanced_dict, cpu_sim, max_steps=30)