...
class CPUSimulator_8Bit:
    def __init__(self, cpu_params_dict): # Nimmt das Dictionary direkt
        # Parameter aus dem Dictionary extrahieren und als Attribute speichern
        self.NUM_REGISTERS = cpu_params_dict["num_registers"]
        self.REGISTER_WIDTH = cpu_params_dict["register_width"]
        self.NUM_INSTRUCTIONS = cpu_params_dict["num_instructions"]
        self.MEMORY_CELLS = cpu_params_dict["memory_cells"]
        self.ADDRESS_BITS = int(np.ceil(np.log2(self.MEMORY_CELLS))) if self.MEMORY_CELLS > 0 else 0
        self.RET_STACK_DEPTH = cpu_params_dict["ret_stack_depth"]
        self.PC_BITS = cpu_params_dict["pc_bits"]
        self.MAX_PC_VAL = (2**self.PC_BITS) - 1
        self.CONTROL_BITS_COUNT = cpu_params_dict["control_bits_count"]

        # Erstelle eine temporäre Dataset-Instanz nur um an die Namen und Konstanten zu kommen
        # Wichtig: Übergebe die Parameter korrekt an den Dummy-Konstruktor
        temp_ds_cpu_config = {
        ...
        # Verwende Dummy-Werte für num_sequences_per_epoch, seq_len, noise_level
...
def execute_program_on_8bit_model_and_sim(model, program_dict, simulator, max_steps=100):
    ...
    for step_count in range(max_steps):
        ...
        predicted_y = model.predict_output(model_input_tensor) # Geändert zu predict_output
        ...
        mem_display_vals = []
        for i in range(min(2, simulator.MEMORY_CELLS)):
...
if __name__ == "__main__":
    ...
    batch_size = 1024
    epochs = 5 # Für den ersten Testlauf beibehalten
    noise_level_data = 0.001
    target_accuracy_threshold = 0.60

    dpp_units = 128
    shared_g_dim_config = input_size // 4 if input_size // 4 > 0 else 1
    ...
    print(f"\nModellparameter: {param_count}") # Mit input_size=75 -> 23385
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', patience=3, factor=0.3, min_lr=1e-8, verbose=True)
    ...
    program_8bit_final_test = {
        ...
        19: (OUTR0B0, 0), # Sollte der PC nach RET sein
        # Subroutine bei SUB_ADDR (PC 20)
        SUB_ADDR + 0: (LDR0IMM, 7),
        SUB_ADDR + 1: (NOTR0, 0),
        SUB_ADDR + 2: (RET, 0),
        SUB_ADDR + 3: (NOOP, 0) # Nur um sicherzustellen, dass der PC nach der Subroutine nicht ins Leere läuft
    }
