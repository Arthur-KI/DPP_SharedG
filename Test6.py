...
TRAINING_NOISE_STD = 0.25 # ÄNDERUNG: Rauschlevel für Training auf 0.25 gesetzt

# --- Hauptskript für den direkten Trainingslauf ---
if __name__ == "__main__":
    run_seed = SEED + 5000 # Neuer Seed für diesen Lauf mit anderem Rauschen
    set_seed(run_seed)
    ...
    trainloader, valloader_clean, _, classes = get_cifar10_dataloaders(
        ...
        download_data=True, training_noise_std=TRAINING_NOISE_STD, test_noise_std=0.0
    )
    ...
    _, history = train_eval_loop(
        model, trainloader, valloader_clean, CRITERION, optimizer, scheduler,
    ...
    _, _, testloader_noisy_0_25_final, _ = get_cifar10_dataloaders( batch_size=BATCH_SIZE, num_workers=NUM_WORKERS_DATALOADER, download_data=False, training_noise_std=0.0, test_noise_std=0.25) # Test mit 0.25 Rauschen
    ...
    eval_sets = {
        ...
        f"Verrauschtes Testset (StdAbw=0.25)": testloader_noisy_0_25_final,
        "Verrauschtes Testset (StdAbw=0.8)": testloader_noisy_0_8_final
    }
    if TRAINING_NOISE_STD not in [0.0, 0.25, 0.8]: # Falls mit einem anderen Level trainiert wurde, dieses auch explizit testen
        _, _, custom_testloader, _ = get_cifar10_dataloaders( batch_size=BATCH_SIZE, num_workers=NUM_WORKERS_DATALOADER, download_data=False, training_noise_std=0.0, test_noise_std=TRAINING_NOISE_STD)
        eval_sets[f"Verrauschtes Testset (StdAbw={TRAINING_NOISE_STD})"] = custom_testloader
    ...
    for desc, current_testloader in eval_sets.items():
        ...
        try: # Extrahiere Rauschlevel für den Plot-Key
            if "StdAbw=" in desc: noise_val_str = desc.split("StdAbw=")[1].split(")")[0]; robustness_results[float(noise_val_str)] = final_test_accuracy
            elif "Sauber" in desc: robustness_results[0.0] = final_test_accuracy
        except: pass # Ignoriere, wenn das Parsen fehlschlägt
