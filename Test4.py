...
# ÄNDERUNG: training_noise_std als Parameter hinzugefügt
def get_cifar10_dataloaders(batch_size=128, num_workers=2, use_advanced_augmentations=True, download_data=True, training_noise_std=0.0):
    print(f"Datenvorbereitung: CIFAR-10. Augmentierung: {use_advanced_augmentations}, Download: {download_data}, Trainings-Rauschen (StdAbw): {training_noise_std}")
    ...
    # ToTensor kommt vor dem Rauschen, Rauschen vor der Normalisierung
    transform_train_list_end = [normalize]
    ...
    transform_train_list = transform_train_list_base + \
                           ...
                           transform_train_list_end

    transform_train = transforms.Compose(transform_train_list)
    transform_test = transforms.Compose([transforms.ToTensor(), normalize]) # Testdaten bleiben sauber
    ...
    # Separater Testloader für verrauschte Testdaten (wird nur für Evaluation genutzt)
    test_transform_noisy_list = [transforms.ToTensor()] + \
                                ([AddGaussianNoise(std=0.8)] if 0.8 > 0 else []) + \
                                [normalize] # Annahme: Wir wollen mit noise_std=0.8 evaluieren
    transform_test_noisy_0_8 = transforms.Compose(test_transform_noisy_list)
    testset_noisy_0_8 = torchvision.datasets.CIFAR10(root='./data_cifar10', train=False, download=download_data, transform=transform_test_noisy_0_8)
    testloader_noisy_0_8 = DataLoader(testset_noisy_0_8, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)


    classes = ('plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck')
    print("Datenvorbereitung abgeschlossen.")
    return trainloader, testloader, testloader_noisy_0_8, classes # Gebe auch den verrauschten Testloader zurück
...
def train_eval_loop(model, trainloader, valloader, criterion, optimizer, scheduler, num_epochs, device,
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for inputs, labels in valloader: # Valloader sollte saubere Daten für Validierung während des Trainings verwenden
                inputs = inputs.to(device, non_blocking=non_blocking_transfer)
                labels = labels.to(device, non_blocking=non_blocking_transfer)
                with torch.amp.autocast(device_type=device.type, enabled=use_amp_local):
                    outputs = model(inputs)
...
NUM_EPOCHS_TRAINING = 50 # Anzahl der Epochen für diesen Testlauf mit Rauschen
USE_AMP_GLOBAL = torch.cuda.is_available()
BATCH_SIZE = 256
NUM_WORKERS_DATALOADER = 4 if DEVICE.type == 'cuda' else 0
TRAINING_NOISE_STD = 0.15 # Standardabweichung des Rauschens für Trainingsdaten (moderat)

# --- Hauptskript für den direkten Trainingslauf ---
if __name__ == "__main__":
    run_seed = SEED + 3000 # Neuer Seed für diesen Lauf
    set_seed(run_seed)
    ...
    # Beste Hyperparameter (aus vorherigem Optuna-Lauf, als Startpunkt)
    best_params = {
        "lr": 0.002003627237023472,
        "optimizer": "AdamW",
        "weight_decay": 1.2138009911679477e-05,
        "base_channels": 64,
        "dropout_conv": 0.0013585700483887998, # Sehr niedrig, evtl. erhöhen bei Training mit Rauschen
    ...
    model = DPP_VFNCIFAR10Net(
        ...
        dropout_conv=best_params["dropout_conv"], dropout_fc=best_params["dropout_fc"], # Eventuell Dropout Conv erhöhen
    ...
    # Datenlader: Trainingsdaten jetzt mit Rauschen
    trainloader, testloader_clean, testloader_noisy_0_8, classes = get_cifar10_dataloaders(
        batch_size=BATCH_SIZE, use_advanced_augmentations=True, num_workers=NUM_WORKERS_DATALOADER,
        download_data=True, training_noise_std=TRAINING_NOISE_STD
    )

    scaler = torch.amp.GradScaler(enabled=(USE_AMP_GLOBAL and DEVICE.type == 'cuda'))

    print(f"\nStarte Training mit verrauschten Daten für {NUM_EPOCHS_TRAINING} Epochen...")
    # Validierung während des Trainings erfolgt auf sauberen Testdaten (testloader_clean)
    _, history = train_eval_loop(
        model, trainloader, testloader_clean, CRITERION, optimizer, scheduler,
    ...
    eval_sets = {
        "Sauberes Testset": testloader_clean,
        "Verrauschtes Testset (StdAbw=0.8)": testloader_noisy_0_8
    }
    if TRAINING_NOISE_STD > 0 and TRAINING_NOISE_STD != 0.8: # Falls mit anderem Rauschen trainiert wurde, dieses auch testen
        _, _, testloader_train_noise_level, _ = get_cifar10_dataloaders(
            batch_size=BATCH_SIZE, num_workers=NUM_WORKERS_DATALOADER,
            download_data=False, training_noise_std=0.0 # Dummy, nur um Loader mit korrektem Rauschen zu bekommen
        )
        # Erstelle den spezifischen Testloader mit dem Trainingsrauschen manuell
        temp_transform = transforms.Compose([transforms.ToTensor()] + \
                                            [AddGaussianNoise(std=TRAINING_NOISE_STD)] + \
                                            [transforms.Normalize(mean=[0.4914, 0.4822, 0.4465], std=[0.2023, 0.1994, 0.2010])])
        temp_testset = torchvision.datasets.CIFAR10(root='./data_cifar10', train=False, download=False, transform=temp_transform)
        testloader_train_noise_level = DataLoader(temp_testset, batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS_DATALOADER, pin_memory=True)
        eval_sets[f"Verrauschtes Testset (StdAbw={TRAINING_NOISE_STD})"] = testloader_train_noise_level
