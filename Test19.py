...
class FPLA_V1_Dataset(IterableDataset):
    def __init__(self, num_sequences_per_epoch, seq_len, noise_level=0.0,
                 num_instructions=16, data_mem_size=4, ret_stack_depth=4): # num_instructions ist 16
        ...
        self.LOAD_R0_X = 0
        self.LOAD_R1_X = 1
        self.LOAD_RA_X = 2        # RA mit x1_t laden (Index 0..data_mem_size-1)
        ...
        self.RETURN_FROM_SUB = 12 # KORRIGIERT: War vorher RETURN
        self.OUT_R0 = 13
    ...
    def __iter__(self):
        SUBROUTINE_START_ADDR = self.seq_len + 10 

        for _ in range(self.num_sequences_per_epoch):
            # R0, R1 sind Datenregister. RA ist Adressregister (Wert 0 bis data_mem_size-1)
            r0, r1, ra_val, f0 = 0.0, 0.0, 0.0, 0.0 
            ...
            while idx_in_sequence < self.seq_len:
                ...
                ra_one_hot = self._to_one_hot(ra_val, self.data_mem_size) # RA als One-Hot
                ...
                # Input: x1(1) + instr(16) + R0(1) + R1(1) + RA_oh(4) + F0(1) + DataMem(4) + SP_oh(4) + y_tm1(1) = 33
                ...
                next_r0, next_r1, next_ra_val, next_f0 = r0, r1, ra_val, f0
                next_data_memory = list(data_memory)
                target_y = 0.0
                next_pc_val = program_counter + 1
                mem_addr_clean = int(ra_val) # ra_val ist der direkte Index 0..data_mem_size-1
                ...
                if current_instr_idx_clean == self.LOAD_R0_X: next_r0 = current_x1_bit_clean
                    ...
                    next_ra_val = float(int(current_x1_bit_clean) % self.data_mem_size)
                ...
                r0, r1, ra_val, f0 = next_r0, next_r1, next_ra_val, next_f0
...
def train_model_amp(model, train_loader, criterion, optimizer, scaler, epochs=100, model_name="Model", target_accuracy=0.98, steps_per_epoch=None, scheduler_obj=None):
    ...
    early_stop_threshold = 15 # Erhöht für diese komplexe Aufgabe
    ...
    for epoch in range(epochs):
        ...
        if accuracy >= 0.999: # Schwelle für Early Stopping etwas niedriger ansetzen
...
if __name__ == "__main__":
    ...
    batch_size = 4096 # Noch größere Batch Size für Stabilität und GPU-Auslastung
    epochs = 500      
    noise_level_data = 0.001 
    target_accuracy_threshold = 0.55 # Sehr, sehr konservatives Ziel

    dpp_units = 96
    shared_g_dim_config = 16 # input_size (33) / 2 = ~16

    print(f"Task: APLU V1 (Fortgeschrittene Programmierbare Logik-Einheit)")
    ...
    num_train_sequences_per_epoch = 100000 # Noch mehr Daten
    num_test_sequences_per_epoch = 20000
    seq_len_task = 80 # Sehr lange Programme
    ...
    # Bei sehr großen IterableDatasets und num_workers > 0 kann es zu Speicherproblemen kommen,
    # daher für diesen extremen Test num_workers=0 belassen.
    ...
    print(f"\nModellparameter: {param_count}") # Sollte 8801 sein für input=33, dpp=96, sgd=16
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', patience=60, factor=0.2, min_lr=1e-8, verbose=True) 

    print(f"\n--- Training DPP SharedG (H={dpp_units}, SG_dim={shared_g_dim_config}) für APLU V1 ---")
    
    epochs_this_run = 5 # Für Ihren schnellen Test, wie im Log
    # epochs_this_run = epochs # Für einen vollen Lauf
    
    history, total_time, epoch_target, time_target = train_model_amp(
        model_to_train, train_loader, criterion, optimizer, scaler,
        epochs=epochs_this_run, model_name="DPP_APLU_V1", target_accuracy=target_accuracy_threshold,
        steps_per_epoch=train_steps_per_epoch, scheduler_obj=scheduler
    )

    print("\n--- Finale Evaluation (APLU V1) ---")
    final_acc, final_loss = evaluate_model_amp(model_to_train, test_loader, criterion, model_name="DPP_APLU_V1", steps_per_epoch=test_steps_per_epoch)
    print(f"DPP_APLU_V1 - Parameter: {param_count}")
    ...
    # Alpha-Inspektion
    print("\n--- Alpha-Inspektion für einige Test-Samples (APLU V1) ---")
    model_to_train.eval()
    max_inspect_alpha_apl_v1 = 30 

    inspector_dataset_apl_v1 = FPLA_V1_Dataset(1, max_inspect_alpha_apl_v1 + seq_len_task, 0.0, num_instructions_task, data_mem_size_task, ret_stack_depth_task) # Saubere Daten
    
    instr_names_apl_v1 = inspector_dataset_apl_v1.instr_names
    ...
    with torch.no_grad():
        insp_r0, insp_r1, insp_ra_val, insp_f0 = 0.0, 0.0, 0.0, 0.0
        ...
        for sample_count_inspect in range(max_inspect_alpha_apl_v1):
            current_insp_x1_clean = float(random.randint(0,1))
            current_instr_idx_clean_insp = 0
            
            if insp_skip_next_n_instr > 0:
                current_instr_idx_clean_insp = inspector_dataset_apl_v1.NO_OP
                insp_skip_next_n_instr -= 1
            elif insp_program_counter == SUBROUTINE_START_ADDR_INSP_V3 : 
                current_instr_idx_clean_insp = inspector_dataset_apl_v1.ALU_NOT_R0
            elif insp_program_counter == SUBROUTINE_START_ADDR_INSP_V3 + 1: 
                current_instr_idx_clean_insp = inspector_dataset_apl_v1.MOVE_R0_R1
            elif insp_program_counter == SUBROUTINE_START_ADDR_INSP_V3 + 2: 
                current_instr_idx_clean_insp = inspector_dataset_apl_v1.RETURN_FROM_SUB
            else:
                current_instr_idx_clean_insp = random.randint(0, num_instructions_task -1)
                if current_instr_idx_clean_insp == inspector_dataset_apl_v1.CALL_SUB_FIXED and insp_ret_sp >= ret_stack_depth_task: 
                    current_instr_idx_clean_insp = inspector_dataset_apl_v1.NO_OP
                if current_instr_idx_clean_insp == inspector_dataset_apl_v1.RETURN_FROM_SUB and insp_ret_sp == 0: 
                    current_instr_idx_clean_insp = inspector_dataset_apl_v1.NO_OP

            current_instr_oh_clean = inspector_dataset_apl_v1._to_one_hot(current_instr_idx_clean_insp, num_instructions_task)
            current_ra_oh_clean = inspector_dataset_apl_v1._to_one_hot(insp_ra_val, data_mem_size_task)
            ...
            if len(model_input_features_clean) != input_size:
                print(f"WARNUNG: Feature-Anzahl in Inspektion ({len(model_input_features_clean)}) vs. input_size ({input_size}). Sample wird übersprungen.")
                continue
            ...
            next_insp_r0, next_insp_r1, next_insp_ra_val, next_insp_f0 = insp_r0, insp_r1, insp_ra_val, insp_f0
            next_insp_data_memory = list(insp_data_memory)
            correct_target_y_insp = 0.0 
            next_insp_pc_val = insp_program_counter + 1
            mem_addr_insp_clean = int(insp_ra_val)

            ds = inspector_dataset_apl_v1 
            ...
            elif current_instr_idx_clean_insp == ds.LOAD_RA_X: next_insp_ra_val = float(int(current_insp_x1_clean) % ds.data_mem_size)
            ...
            ra_oh_str_disp = "".join(map(str, [int(s) for s in current_ra_oh_clean])) 
            sp_oh_str_disp = "".join(map(str, [int(s) for s in current_sp_oh_clean]))

            print(f"S{sample_count_inspect+1}: x1:{int(current_insp_x1_clean)},I:{instr_display_name}({current_instr_idx_clean_insp})|R0i:{int(insp_r0)},R1i:{int(insp_r1)},RAi_oh:{ra_oh_str_disp}(v {int(insp_ra_val)}),F0i:{int(insp_f0)},DM:[{dm_str}],SPi_oh:{sp_oh_str_disp}(v {insp_ret_sp % ds.ret_stack_depth}),y-1i:{int(insp_previous_y)}|Py:{int(model_output_y)},Ty:{int(correct_target_y_insp)},MA:{mean_alpha:.3f}")
            
            insp_r0, insp_r1, insp_ra_val, insp_f0 = next_insp_r0, next_insp_r1, next_insp_ra_val, next_insp_f0
